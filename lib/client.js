window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const IconTrashOutline16 = icon('IconTrashOutline16', 'IconTrashOutlineRegular');
		const IconArchiveOutline20 = icon('IconArchiveOutline20', 'IconArchiveOutlineRegular');
		const IconWarningOutline16 = icon('IconWarningOutline16', 'IconWarningOutlineRegular');
		const IconCloseOutline16 = icon('IconCloseOutline16', 'IconCloseOutlineRegular');
		const IconRefreshOutline14 = icon('IconRefreshOutline14', 'IconRefreshOutlineRegular');
...
.drb-tab:hover { background: var(--dsw-alias-interactive-bg-hover); color: var(--dsw-alias-label-primary); }
...
.drb-pager-btn:disabled { opacity: .4; cursor: default; }
.drb-pager-info { font-size: 12px; color: var(--dsw-alias-label-tertiary); }
.drb-group-rows {
  border: 1px solid var(--dsw-alias-border-l2);
  border-radius: 10px;
  overflow: hidden;
...
.drb-row:hover { background: var(--dsw-alias-interactive-bg-hover); }
...
body[data-ds-dark-theme] .drb-batch-bar,
body[data-ds-dark-theme] .drb-batch-menu,
body[data-ds-dark-theme] .drb-count {
  background: var(--dsw-alias-bg-layer-3);
}
body[data-ds-dark-theme] .drb-error,
body[data-ds-dark-theme] .drb-running-badge,
body[data-ds-dark-theme] .drb-running-list,
body[data-ds-dark-theme] .drb-row-action-danger,
body[data-ds-dark-theme] .drb-confirm-warning {
  color: var(--dsw-static-red-400);
}
body[data-ds-dark-theme] .drb-row-action-danger { color: var(--dsw-static-red-400) !important; }
body[data-ds-dark-theme] .drb-confirm-warning {
  border-color: rgba(242, 90, 90, .4);
...
`;
		...
		function RecycleBinPanel({ open, onClose, t, getSessions, subscribeSessions, getWorkspaces, subscribeWorkspaces, refreshSessions, refreshWorkspaces }) {
			...
			const renderGroup = (group) => {
				...
				const page = Math.min(groupPages[group.key] ?? 0, totalPages - 1);
				const pageRows = group.rows.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE);
				const setPage = (next) => setGroupPages((prev) => ({ ...prev, [group.key]: next }));
				return h("div", { className: "drb-group", key: group.key },
					...
					h("div", { className: "drb-group-rows" }, pageRows.map(renderRow)),
					totalPages > 1 ? h("div", { className: "drb-pager" },
						h("button", {
							type: "button",
							className: "drb-pager-btn",
							disabled: page === 0,
							onClick: () => setPage(page - 1),
						}, "\u2039"),
						h("span", { className: "drb-pager-info" }, `${page + 1} / ${totalPages}`),
						h("button", {
							type: "button",
							className: "drb-pager-btn",
		...
		function apply(ctx) {
			...
			// DSH 0.1.7 dropped `ctx.workspaces.refresh()`: the Workspace Controller
			// now publishes its snapshot through the follow-driven `list` store, so
			// the manual refresh degrades to a no-op instead of a TypeError.
			const refreshSessions = () => (typeof ctx.sessions.refresh === "function"
				? ctx.sessions.refresh()
				: Promise.resolve());
			const refreshWorkspaces = () => (typeof ctx.workspaces.refresh === "function"
