 ...
 *
 * DSH keeps one directory per session at
 * `<DSH_HOME>/sessions/<workspace-slug>/<session-id>/`, holding the compressed
 * log (`session.jsonl.zstd`) and a `session.lock`. Sessions that were created
 * but never materialized have no directory at all.
 *
 * @param home - resolved DSH home directory.
 * @param id - session id, which is also the directory name (`session-<uuid>`).
...
async function findSessionDir(home, id) {
  ...
  for (const entry of entries) {
    ...
    const candidate = path.join(root, entry.name, id)
    try {
...
export { createMutex, dshHome, findSessionDir, parseSessionIds, purgeSessions, readState, stateFile, unarchiveSessions, writeState }
